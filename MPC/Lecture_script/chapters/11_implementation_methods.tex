...
\[
    \cX_f=\R^2.
\]
The resulting state space is partitioned into finitely many critical regions. The first optimal input is represented by a piecewise affine surface over the state space. The
...
\begin{figure}[!h]
    ...
    \caption{Logarithmic search for explicit MPC. The left part of shows the
...
\[
    ...
    \qquad \kappa>0.
\]
Equivalently, the relaxed system is written as
\[
\begin{aligned}
    \nabla f(x^\star)
    +
