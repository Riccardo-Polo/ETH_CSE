...
\]
Equivalently, the correction term is proportional to the output prediction error. The innovation is written as
\[
...
The plant produces the measured output \(y\). The estimator uses \(y\) and the applied input \(u\) to
...
We illustrate the construction on the double integrator. The first set point is \(r=0\),
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.35\textwidth]{images/quadratic_penalty_hard_problem.png}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.35\textwidth]{images/quadratic_penalty_soft_problem.png}
...
We illustrate the effect of different slack penalties on a third-order nonminimum-phase
system:
\[
    x(k+1)
    =
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.5\textwidth]{images/soft_constraints_example_nominal.png}
...
The simulation uses the initial condition
\[
    x(0)=
    \begin{bmatrix}
        1.5 & 1.5 & 1.5
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.5\textwidth]{images/soft_constraints_quadratic_penalty.png}
...
whenever possible.\\
We first show a case with initial condition
\[
    x(0)=
    \begin{bmatrix}
        0.95 & 0.95 & 0.95
...
Instead of combining performance and violation penalties in one optimization problem, one can separate the objectives into two steps.\\
