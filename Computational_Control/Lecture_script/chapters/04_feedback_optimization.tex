\subsection{Optimal steady-state regulation}
...
The key assumption is that the plant is \emph{exponentially stable}. This means that, for constant inputs $u$ and constant disturbances $w$, the state converges exponentially fast to a unique equilibrium. Therefore, after transients have died out, the output is determined only by the constant values of $u$ and $w$. This motivates the introduction of the \emph{steady-state map}
\[
y = Hu + Lw.
\]
...
\subsection{``Certainty-equivalence'' design}
...
\begin{figure}[!h]
    \centering
    \input{images/feedback_opt_01.tex}
...
\subsection{Interconnecting plant and algorithms}
...
\begin{figure}[!h]
    \centering
    \input{images/feedback_opt_02.tex}
...
\subsection{Online feedback optimization}
...
\subsection{Gradient descent with penalty / barrier}
...
\begin{figure}[!h]
    ...
    \begin{subfigure}{0.45\textwidth}
        \centering
        \input{images/feedback_opt_03.tex}
        \caption{Penalty function.}
    \end{subfigure}
    \hfill
    \begin{subfigure}{0.45\textwidth}
        \centering
        \input{images/feedback_opt_04.tex}
...
\subsection{Unconstrained gradient descent}
...
\subsection{Gradient descent with penalty}
...
\Bigr).
\]
This is the ideal algorithm one would write if the steady-state map $h$ were exactly known. However, in feedback optimization we do not want to evaluate the cost only on the basis of the model output $h(u_k;w)$; rather, we want to use the \emph{measured} plant output. Denoting by $y_k$ the measured output corresponding to the current input $u_k$, the update law becomes
\[
u_{k+1}
=
u_k
...
\begin{figure}[!h]
    \centering
    \input{images/feedback_opt_05.tex}
...
\begin{figure}[!h]
    \centering
    \input{images/feedback_opt_06.tex}
...
\subsection{Projected gradient flow via repeated quadratic programming}
...
\subsection{Dual ascent for feedback optimization}
...
\subsection{An old friend: dual ascent and PI control}
...
\begin{figure}[!h]
    \centering
    \input{images/feedback_opt_07.tex}
...
\subsection{Design guidelines and tradeoffs}
...
\subsection{Extremum-seeking architecture}
...
\begin{figure}[!h]
    \centering
    \input{images/feedback_opt_11.tex}
...
\subsection{Why extremum seeking behaves like gradient descent}
...
\subsection{Applications of extremum seeking}
