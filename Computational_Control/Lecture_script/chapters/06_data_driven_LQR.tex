...
+ (A+BK)^2\,\mathbb E[x_0x_0^\top]\,\big((A+BK)^\top\big)^2 
+ \cdots \\
&= \mathbb{E} [x_0x_0^\top] + \sum_{k=1}^{\infty} (A+BK)^k \mathbb{E}[x_0x_0^\top] ((A+BK)^\top)^k \\
&= \mathbb E[x_0x_0^\top] + (A+BK)\,P\,(A+BK)^\top.
\end{align*}
...
$$
(A+BK)P(A+BK)^\top = (A+BK)\,\mathbb E[x_0x_0^\top]\,(A+BK)^\top + (A+BK)^2\,\mathbb E[x_0x_0^\top]\,\big((A+BK)^\top\big)^2 + \cdots
$$
Using $\mathbb E[x_0x_0^\top]=I$, we obtain the \textbf{Lyapunov equation}
\[
P = I + (A+BK)P(A+BK)^\top.
\]
...
\[
\mathrm{Tr}(x_t^\top (Q + K^\top R K)x_t) = \mathrm{Tr}((Q + K^\top R K)x_t x_t^\top).
\]
