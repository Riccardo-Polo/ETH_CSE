...
\begin{itemize}
    \item Decision space $\mathcal{\tilde{U}}$ is convex, or maybe finite.
...
\end{align*}
To complete the proof one should check that $P_t$ is positive semidefinite. Here we homit the proof.\\
In cocnlusion we have been able to derive a closed-form expression for the optimal control input $u^*$ for the LQR problem.Be aware that this solution is not valid anymore if the problem becomes constrained.\end{proof} 
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item $m\times m$ matrix inversions to compute $\Gamma_t$ and $P_t$ for $t=T-1,\dots,0$
...
converges, for $t \to -\infty$, to a unique positive definite solution \(P_{\infty} \succ 0\) of the algebraic Riccati equation
